...
def load_legal_documents(data_dir: str) -> List[str]:
    ...
    txt_files = sorted(
        [f for f in data_path.glob("*.txt") if not f.name.startswith(".")]
    )
    ...
    for file_path in txt_files:
        try:
            ...
            if len(content) > 100:
                ...
                print(
                    f"  Skipping short document: {file_path.name} ({len(content)} chars)"
                )
    ...
    for doc_idx, doc in enumerate(documents):
        ...
        for i in range(0, len(tokens), stride):
            ...
            tokenized_examples.append(
                {"input_ids": window, "attention_mask": [1] * max_length}
            )

        if (doc_idx + 1) % 10 == 0:
            print(
                f"  Processed {doc_idx + 1}/{len(documents)} documents ({len(tokenized_examples)} sequences)"
            )
...
def split_train_val(
    examples: List[dict], train_ratio: float = 0.9
    ...
    print(
        f"Split: {len(train_examples)} training, {len(val_examples)} validation examples"
    )
    return train_examples, val_examples


def save_preprocessed_data(
    train_examples: List[dict], val_examples: List[dict], output_dir: str
):
    """
...
def main():
    ...
    tokenized_examples = tokenize_documents(
        documents, tokenizer, max_length=MAX_LENGTH, stride=STRIDE
    )
