...
def clean_legal_text(text: str) -> str:
    ...
    for line in content_lines:
        ...
        if not line:
            ...
            if (
                filtered_lines and filtered_lines[-1]
            ):  # Only add blank if previous line wasn't blank
...
def clean_data_file(file_path: Path) -> tuple[bool, bool]:
    ...
    try:
        ...
        if len(cleaned_content) < 100:
            print(
                f"  Skipping {file_path.name}: cleaned content too short ({len(cleaned_content)} chars)"
            )
...
def main():
    ...
    for i, file_path in enumerate(txt_files, 1):
        ...
        if (i % 100 == 0) or (i == len(txt_files)):
            print(
                f"  Progress: {i}/{len(txt_files)} files processed ({cleaned_count} cleaned, {skipped_count} skipped, {error_count} errors)"
            )
